...
cc_df = pd.read_csv("default-of-credit-card-clients-2.csv", header=1)
if 'ID' in cc_df.columns:
    cc_df = cc_df.drop('ID', axis=1)
if 'default payment next month' in cc_df.columns:
    cc_df = cc_df.rename(columns={'default payment next month': 'target'})
cc_df = cc_df.apply(pd.to_numeric, errors='coerce')
X_cc = cc_df.drop('target', axis=1)
y_cc = cc_df['target']
print(list(X_cc.columns))  # Hier solltest du die echten Namen sehen!
...
# Berechnung der Koeffizienten und Namen für Taiwan Credit
features_cc = X_cc.columns
coefs_cc = lr_c_pipe.named_steps['lr'].coef_[0]
coef_df_cc = pd.DataFrame({'Feature': features_cc, 'Koeffizient': coefs_cc})
coef_df_cc_sorted = coef_df_cc.sort_values('Koeffizient')
